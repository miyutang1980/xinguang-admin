...
console.log({loginRegression:'PASS',checks:['single-flight automatic/manual login','master route + records','assignment route + records','semester history','repeat initialization preserves route','background sync preserves route','logout ignores stale response','failed login permits retry'],authRequests:authRoutes.length,productionWrites:0});

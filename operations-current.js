...
(function (root) {
  ...
  const links = [
    ...
    ['校務','家長綁定審核','https://admin.taipingxinguang.org/?section=parentBinding','後台登入後直接審核；限行政／管理員'],
  ...
  const contextual = [
    ...
    ['公告審核','https://admin.taipingxinguang.org/announce-approve/','row；拒絕時 action=reject','2026-09-23 後台新入口 HTTP 200。登入後核對全文，再確認同意或拒絕；不轉傳審核連結'],
    ['公告審核舊通知',base+'/announce-approve/','保留 row 與 action','舊 LINE 卡片入口仍為 404；程式庫已補頁，但正式 Cloudflare 路由尚待部署或轉址，不代表已恢復']
  ];
  const chapters = [
    {title:'版本與上線狀態',paras:[
    ...
    {title:'首次綁定與校方核驗',paras:[
      '家長免輸入綁定碼，可一次申請 1–5 位孩子；爸爸、媽媽或照顧者各自使用自己的 LINE。申請不會自動授權。',
      '行政／管理員登入本後台，點「請假紀錄」下方的「家長綁定審核」，沿用登入，不需另填帳密；教師與業務不能審核。',
      '透過校方既有聯絡管道核驗身分，逐位選取正確學生，勾選核驗聲明後「一次核准 N 位孩子」。確認視窗會列出學生姓名與編號，確認前不會寫入。',
      '資料不符時退回整份並填原因；核准後請家長重新讀取家長中心。已處理申請保留最近 50 份；撤銷只移除該申請新增授權。',
      '一次性綁定碼保留為備用方式，有效 30 分鐘，不取代一般免碼申請。不可手動把試算表狀態改成已核准，或整批清空既有 LINE 關係。'
    ]},
